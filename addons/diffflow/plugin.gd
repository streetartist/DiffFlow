...
func _on_connection_changed(connected: bool) -> void:
	if connected:
		...
		_broadcast_open_scenes_if_changed(true)
	else:
		_file_watcher.stop()
...
func _on_remote_file_deleted(payload: Dictionary) -> void:
	...
	if _is_open_scene(res_path):
		...
		await _close_scene_path(rel_path)
...
func _on_remote_scene_takeover_requested(payload: Dictionary) -> void:
	...
	dialog.confirmed.connect(func() -> void:
		dialog.queue_free()
		_approve_scene_takeover(rel_path, requester_peer_id, requester_username)
	)
	dialog.canceled.connect(func() -> void:
...
func _approve_scene_takeover(rel_path: String, requester_peer_id: String, requester_username: String) -> void:
	...
	var save_err: int = await _save_active_scene(rel_path)
	if save_err != OK:
...
func _on_remote_scene_takeover_approved(payload: Dictionary) -> void:
	...
	if rel_path.is_empty():
		return
	_open_latest_scene_after_takeover(rel_path, str(payload.get("sha256", "")))
