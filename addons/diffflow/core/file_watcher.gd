...
func _initial_sync() -> void:
	...
	for rel_path in server_files:
		...
		if not FileAccess.file_exists(full_path):
			...
			continue
		var local_hash := _file_sha256(full_path)
	...
	for rel_path in _file_cache.keys():
...
func _to_rel_path(full_path: String) -> String:
	var normalized := full_path.replace("\\", "/")
...
func _scan_dir(path: String, initial: bool, out: Dictionary = {}) -> void:
	...
	while fname != "":
		if fname == "." or fname == "..":
			fname = dir.get_next()
			continue
		var full_path := path.path_join(fname).replace("\\", "/")
		var rel_path := _to_rel_path(full_path)
...
func _upload_file(rel_path: String) -> void:
	...
	var path := "/api/projects/%d/files?path=%s&mtime=%d" % [_project_id, rel_path.uri_encode(), mtime]
...
func _delete_remote(rel_path: String) -> void:
	if _server_url.is_empty() or _token.is_empty() or _project_id <= 0:
		return
	var mtime := int(Time.get_unix_time_from_system())
	var path := "/api/projects/%d/files?path=%s&mtime=%d" % [_project_id, rel_path.uri_encode(), mtime]
...
func _download_file(rel_path: String) -> void:
	if _server_url.is_empty() or _token.is_empty() or _project_id <= 0:
		return
	var path := "/api/projects/%d/files?path=%s" % [_project_id, rel_path.uri_encode()]
	_transferring[rel_path] = true
...
func _reload_in_editor(res_path: String) -> void:
	...
	if res_path.ends_with(".tscn") or res_path.ends_with(".scn"):
		...
		if ResourceLoader.exists(res_path):
			var script := ResourceLoader.load(res_path, "", ResourceLoader.CACHE_MODE_REPLACE)
...
func _request(method: int, path: String, headers: PackedStringArray, body: PackedByteArray) -> Dictionary:
	...
	if code < 200 or code >= 300:
		var message := response_body.get_string_from_utf8()
